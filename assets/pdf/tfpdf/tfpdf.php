...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if ($uni) {
		...
		$info['i'] = count((array)$this->fonts)+1;
		if(!empty($info['diff']))
		{
...
function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='')
{
	...
	if(!isset($this->images[$file]))
	{
		...
		$info = $this->$mtd($file);
		$info['i'] = count((array)$this->images)+1;
...
function _putTTfontwidths(&$font, $maxUni) {
	...
	$w = '';
	foreach ($range as $k => $ws) {
		if (count((array)array_count_values($ws)) == 1) { $w .= ' '.$k.' '.($k + count((array)$ws) - 1).' '.$ws[0]; }
		else { $w .= ' '.$k.' [ '.implode(' ', $ws).' ]' . "\n"; }
	}
	$this->_out('/W ['.$w.' ]');
}
...
function _putimage(&$info)
{
	...
	{
		$trns = '';
		for($i=0;$i<count((array)$info['trns']);$i++)
