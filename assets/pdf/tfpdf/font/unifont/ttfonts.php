...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	function makeSubset($file, &$subset) {
		...
		$numGlyphs = $numberOfHMetrics = count((array)$subsetglyphs );
		...
		$segCount = count((array)$range) + 1;	// + 1 Last segment has missing character 0xFFFF
		$searchRange = 1;
		$entrySelector = 0;
		...
		foreach($range AS $start=>$subrange) {
			$endCode = $start + (count((array)$subrange)-1);
	...
	function getGlyphData($originalGlyphIdx, &$maxdepth, &$depth, &$points, &$contours) {
		...
		if (count((array)$this->glyphdata[$originalGlyphIdx]['compGlyphs'])) {
		...
		if ($numberOfContours < 0) {
			...
			while ($flags & GF_MORE) {
				...
				if (!isset($glyphSet[$glyphIdx])) {
					$glyphSet[$glyphIdx] = count((array)$subsetglyphs);	// old glyphID to new glyphID
	...
	function endTTFile(&$stm) {
		$stm = '';
		$numTables = count((array)$this->otables);
		$searchRange = 1;
		$entrySelector = 0;
